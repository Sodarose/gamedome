...
public class PlayerFactory {
    ...
    public static Player createPlayerEntity(PlayerModel playerModel){
        ...
        // 基础等级属性设置
        DictRoleLevelProperty dictRoleLevelProperty = dictionaryManager.getDictRoleLevelProperty(playerModel.getCareer(), playerModel.getLevel());
        if(dictRoleLevelProperty==null){
            logger.error("为找到该职业ID为{},等级为{}的基础属性", playerModel.getCareer(), playerModel.getLevel());
            return null;
        }
        player.setLevelProperty(dictRoleLevelProperty);
        // 基础属性设置
        Property property = new Property(dictRoleLevelProperty);
